...
const Dashboard: React.FC = () => {
    ...
    useEffect(() => {
        ...
        if (!myChart.getOption() || !isChartInitialized) { 
            ...
            // 数据格式转换
            const dates = allChartData.map(item => new Date(item[0]).toLocaleTimeString());
            ...
            const option: EChartsOption = {
                ...
                dataZoom: [
                    ...
                    { type: 'slider', xAxisIndex: [0, 1], start: 90, end: 100, bottom: 10, height: 20 }
                ],
                
                grid: [
                    { left:'3%', right:'3%', top: '10%', height:'50%' }, 
                    { left:'3%', right:'3%', height:'15%', bottom:'10%' } 
                ],
                xAxis: [
                ...
                series: [
                    { name: 'K线图', type: 'candlestick', data: values, xAxisIndex: 0, yAxisIndex: 0, 
                        itemStyle: { color: '#ef232a', color0: '#14b281', borderColor: '#ed232a', borderColor0: '#14b281' }
                    },
                    { name: '交易量', type: 'bar', data: volumes, xAxisIndex: 1, yAxisIndex: 1, 
        ...
        return () => {
            const instance = echarts.getInstanceByDom(chartRef.current as HTMLDivElement);
            if (instance) {
                instance.dispose(); 
    ...
    // ------------------------------------------------------------------
    // 3. ⭐️ 核心：实时 K 线和交易量增量更新
    // ------------------------------------------------------------------
    useEffect(() => {
        // 确保图表初始化完成且有有效价格
        if (!isChartInitialized || currentPrice <= 0 || allChartData.length === 0 || !chartRef.current) {
            return;
        }

        const myChart = echarts.getInstanceByDom(chartRef.current as HTMLDivElement);
        if (!myChart) return;
        
        // 🚨 必须深拷贝数据，避免直接修改 ECharts 内部数据结构
        const currentOption = myChart.getOption() as EChartsOption; 
        
        // 获取当前的 X 轴、K 线和交易量数据
        const xAxisData = currentOption.xAxis?.[0]?.data as string[] || [];
        ...
        if (isSameBar) { 
            ...
            const newCandle: number[] = [currentPrice, currentPrice, currentPrice, currentPrice];
